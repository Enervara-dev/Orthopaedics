...
from graphrag.domain.query_taxonomy import INTENT_TO_QUERYTYPE
from graphrag.domain.vocabulary import CLINICAL_STATE_KEYS
...
# Map gatekeeper intent strings to QueryType, built from the domain taxonomy
# (graphrag/domain/query_taxonomy.py::INTENT_TO_QUERYTYPE). Intents intentionally
# omitted (greeting, followup_query, emergency, unknown) are short-circuited or
# routed to NO_RETRIEVAL/MEMORY_FIRST before QueryType is consulted, so the
# absence of a mapping is the correct signal.
GATEKEEPER_INTENT_TO_QUERYTYPE: dict[str, QueryType] = {
    intent: QueryType(qt_value) for intent, qt_value in INTENT_TO_QUERYTYPE.items()
}
...
def _has_extracted_entities(analysis: dict[str, Any]) -> bool:
    ...
    return any(entities.get(k) for k in CLINICAL_STATE_KEYS)
